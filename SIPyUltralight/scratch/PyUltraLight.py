...
def evolve(central_mass, num_threads, length, length_units, resol, duration, duration_units, step_factor, save_number, save_options,
    ...
    psi, rho = RandomPhase(128, 10,  1, 1 ,masked = True)
    ...
    for ix in range(actual_num_steps):
        if halfstepornot == 1:
            psi = ne.evaluate("exp(-1j*0.5*h*(phisp-lam*real(abs(psi)**2)))*psi") #SELF INTERACTION HAS BEEN ADDED: JB-edit
            halfstepornot = 0
        else:
            psi = ne.evaluate("exp(-1j*h*(phisp-lam*real(abs(psi)**2)))*psi") #SELF INTERACTION HAS BEEN ADDED: JB-edit
        funct = fft_psi(psi)
