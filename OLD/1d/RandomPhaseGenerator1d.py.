...
#         k_vals = np.linspace(0, 2*k_0, N_modes)
    k_vectors = zeros((1,N_modes)) # [spacial component, k mode index]
    thetas = zeros((N_modes), dtype=float) # [field index, k mode index]
    psi = zeros(N_steps, dtype=complex)
    n = 0 # nth mode
    for q in k_vals: # plane wave in each field for each k value
        ...
        #k_vectors[:, n] =  q #[q*cos(k_angle), q*sin(k_angle)]
        ...
        psi += exp(-q**2 / k_0 ** 2) * exp(1j * thetas[n]) * exp(1j * (q * X))
#             print(thetas[j,i])
        n += 1
